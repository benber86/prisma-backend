...
class CurveMetaPool(CurvePool):  # pylint: disable=too-many-instance-attributes
    """
    ...
    def exchange_underlying(self, i, j, dx):
        ...
        if base_i < 0 or base_j < 0:  # if i or j not in basepool
            ...
            if base_j >= 0:
                dy, dy_fee = self.basepool.remove_liquidity_one_coin(
                    dy, base_j
                )
    ...
    def get_dy_underlying(self, i, j, dx):
        ...
        if base_i < 0 or base_j < 0:  # if i or j not in basepool
            ...
            # Withdraw from the base pool if needed
            if base_j >= 0:
                dy, dy_fee = self.basepool.calc_withdraw_one_coin(dy, base_j)
    ...
    def add_liquidity(self, amounts):
        ...
        new_balances = [
            bal + amt - fee
            for bal, amt, fee in zip(balances, amounts, admin_fees)
        ]
        self.balances = new_balances
        self.admin_balances = [
            t + a for t, a in zip(self.admin_balances, admin_fees)
        ]
